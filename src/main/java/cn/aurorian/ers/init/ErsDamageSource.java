...
public class ErsDamageSource {
    public static final ResourceKey<DamageType> DIE_OF_BLEED =
            ResourceKey.create(Registries.DAMAGE_TYPE, EcologicalReplenishmentStation.prefix("bleeding"));

    public static DamageSource getDamageSource(Level level, ResourceKey<DamageType> type) {
        return new DamageSource(
                level.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(type), null, null);
    }
}
