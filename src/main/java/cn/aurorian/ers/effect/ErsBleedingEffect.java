...
public class ErsBleedingEffect extends MobEffect {
    private static final Map<LivingEntity, Vec3> previousPositions = new WeakHashMap<>();
    private static final Map<LivingEntity, Boolean> still = new WeakHashMap<>();
    ...
    public void applyEffectTick(@NotNull LivingEntity pLivingEntity, int pAmplifier) {
        ...
        if (still.get(pLivingEntity) != null && still.get(pLivingEntity)) {
            ...
            if (amp != 0) {
                pLivingEntity.addEffect(
                        new MobEffectInstance(this, instance.getDuration(), amp - 1, false, false, true));
            }
        }
        ...
        if (!previousPositions.isEmpty() && previousPosition != null && previousPosition.equals(currentPosition)) {
            ...
            if (pLivingEntity.isSprinting()) modifier = 2f;
            still.put(pLivingEntity, false);
        }

        pLivingEntity.hurt(
                ErsDamageSource.getDamageSource(pLivingEntity.level(), ErsDamageSource.DIE_OF_BLEED),
                pLivingEntity.getMaxHealth() * 0.01f * (pAmplifier + 1) * modifier);
    ...
    public void removeAttributeModifiers(
            @NotNull LivingEntity pLivingEntity, @NotNull AttributeMap pAttributeMap, int pAmplifier) {
        super.removeAttributeModifiers(pLivingEntity, pAttributeMap, pAmplifier);
        previousPositions.remove(pLivingEntity);
        still.remove(pLivingEntity);
    }
        ...
        if (instance == null) {
            ...
            if (currentAmp + step < maxAmp) {
                instance.update(new MobEffectInstance(
                        ErsMobEffects.BLEEDING.get(), 1200, currentAmp + step, false, false, true));
    ...
    public static void removeBleedingEffect(LivingEntity target) {
        ...
        if (currentAmp > 0) {
            target.addEffect(
                    new MobEffectInstance(ErsMobEffects.BLEEDING.get(), duration, currentAmp - 1, false, false, true));
        }
    }
}
