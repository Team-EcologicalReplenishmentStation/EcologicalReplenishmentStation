...
public class GeneralVehicleBodyControl extends BodyRotationControl {
    ...
    protected boolean vehicleTick(float walkRotSpeed, float sprintRotSpeed, int startAttackTurningTick, float yHeadRotSpeed, float xRotSpeed, float xRotLimit,float maxHeadYRotDegree){
        if(vehicle.getControllingPassenger() instanceof Player driver && vehicle.level().isClientSide) {
            if(vehicle.getAttackState().getType().isTurning())
                tickCount++;
            ...
            if (vehicle.getAttackState().getType().canMove() && (Math.abs(driver.xxa) > 0 || Math.abs(driver.zza) > 0)) {
                ...
                if(Mth.degreesDifference(targetYaw, this.vehicle.getYRot()) > 0 && vehicle.isControlledByLocalInstance()) {
                    this.vehicle.setRotDirection(MobRotDirection.of(MobRotDirection.RotDirection.LEFT, isLargeTurn),true);
                } else if(vehicle.isControlledByLocalInstance()){
                    this.vehicle.setRotDirection(MobRotDirection.of(MobRotDirection.RotDirection.RIGHT, isLargeTurn),true);
                }
    ...
    protected void aiTick(float maxHeadYRotDegree){
        if(vehicle.getControllingPassenger() == null){
            ...
            if(this.vehicle.isMoving())
                this.vehicle.yHeadRot = this.vehicle.getYRot();
