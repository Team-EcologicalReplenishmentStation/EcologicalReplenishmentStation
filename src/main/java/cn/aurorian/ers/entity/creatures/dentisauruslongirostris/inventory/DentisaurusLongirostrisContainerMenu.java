...
public class DentisaurusLongirostrisContainerMenu extends AbstractMountContainerMenu<DentisaurusLongirostrisEntity> {

    public DentisaurusLongirostrisContainerMenu(
            int pContainerId, Inventory pPlayerInventory, DentisaurusLongirostrisEntity entity) {
        super(ErsContainers.SWAMP_DRAGON_CONTAINER.get(), pContainerId, pPlayerInventory, entity);
    }
    ...
    protected void addEntitySlots() {
        ...
        if (entity.isElite()) {
            ...
            this.addSlot(new Slot(container, 4, 8, 54) {
                @Override
                public boolean mayPlace(@NotNull ItemStack stack) {
                    return stack.is(Items.GOLDEN_HORSE_ARMOR);
                }
        ...
        this.addSlot(new Slot(container, 5, 98, 39) {
            @Override
            public boolean mayPlace(@NotNull ItemStack stack) {
                return stack.is(ErsItems.FISH_FEED.get());
            }
        });
    ...
    protected boolean moveToEntitySlots(ItemStack currentItem, ItemStack slotStack) {
        ...
        } else if (currentItem.is(Items.GOLDEN_HORSE_ARMOR) && entity.isElite()) {
