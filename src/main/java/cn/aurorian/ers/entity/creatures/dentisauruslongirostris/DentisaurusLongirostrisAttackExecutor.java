...
public class DentisaurusLongirostrisAttackExecutor {
    ...
    public void executeAttack(MobAttack attack, DentisaurusLongirostrisEntity mount) {
        ...
        if (attack.animatorTick == AttackType.SWAMP_DRAGON_ATTACK.getAnimationLength() - 9) {
            ...
            for (Entity entity : entities) {
                ...
                if (mount.level() instanceof ServerLevel serverLevel) {
                    serverLevel.sendParticles(ErsParticleType.BLOOD.get(), x, y, z, 20, 0.3, 0.3, 0.3, 0.5);
                }
            }
        }
    }

    public void executeJudgement(MobAttack attack, DentisaurusLongirostrisEntity mount) {
        if (attack.judgementTarget == null) {
            ...
            if (!entities.isEmpty()) {
                ...
                if (closestEntity != null) {
                    ...
                    attack.judgementTarget =
                            (LivingEntity) entities.get(mount.level().random.nextInt(entities.size()));
                }

                if (swim) {
                    mount.triggerAnim("attack", "swim_attack");
                    attack.triggerTime = 40;
                } else if (attack.judgementTarget.getDimensions(Pose.STANDING).height
                                * attack.judgementTarget.getDimensions(Pose.STANDING).width
                                * attack.judgementTarget.getDimensions(Pose.STANDING).width
                        <= 0.648) {
    ...
    public void executeSpecialAttack(MobAttack attack, DentisaurusLongirostrisEntity mount) {
        if (attack.animatorTick == AttackType.SWAMP_DRAGON_SPECIAL_ATTACK.getAnimationLength() - 13
                || attack.animatorTick == AttackType.SWAMP_DRAGON_SPECIAL_ATTACK.getAnimationLength() - 29
                || attack.animatorTick == AttackType.SWAMP_DRAGON_SPECIAL_ATTACK.getAnimationLength() - 49) {
            ...
            for (Entity target : entities) {
                if (target.equals(mount.getControllingPassenger()) || mount.isAlliedTo(target)) continue;
            ...
            if (mount.level() instanceof ServerLevel serverLevel) {
                for (int i = 0; i < 10; i++) {
                    double particleX =
                            attackBox.minX + mount.getRandom().nextDouble() * (attackBox.maxX - attackBox.minX);
                    double particleY =
                            attackBox.minY + mount.getRandom().nextDouble() * (attackBox.maxY - attackBox.minY);
                    double particleZ =
                            attackBox.minZ + mount.getRandom().nextDouble() * (attackBox.maxZ - attackBox.minZ);
                    if (mount.isInWater()) {
                        serverLevel.sendParticles(
                                ParticleTypes.BUBBLE, particleX, particleY, particleZ, 6, 0.3, 0.3, 0.3, 0.1);
                    } else {
                        serverLevel.sendParticles(
                                ParticleTypes.FIREWORK, particleX, particleY, particleZ, 6, 0.3, 0.3, 0.3, 0.1);
                    }
                }
            }
        }
    ...
    public void executeTurningAttack(MobAttack attack, DentisaurusLongirostrisEntity mount) {
        if (attack.animatorTick == AttackType.SWAMP_DRAGON_ATTACK_TURN.getAnimationLength() - 20
                || attack.animatorTick == AttackType.SWAMP_DRAGON_ATTACK_TURN.getAnimationLength() - 10) {
        ...
        if (attack.animatorTick == AttackType.SWAMP_DRAGON_ATTACK_TURN.getAnimationLength() - 16
                || attack.animatorTick == AttackType.SWAMP_DRAGON_ATTACK_TURN.getAnimationLength() - 18
                || attack.animatorTick == AttackType.SWAMP_DRAGON_ATTACK_TURN.getAnimationLength() - 20) {
            mount.setDeltaMovement(attack.turningAttackDirection.x * 1.7, 0, attack.turningAttackDirection.z * 1.7);
        }
    }

    public void executeCatchFishSmall(MobAttack attack, DentisaurusLongirostrisEntity mount) {
        if (attack.getType() != AttackType.SWAMP_DRAGON_CATCH_FISH_SMALL) return;

        if (attack.animatorTick == AttackType.SWAMP_DRAGON_CATCH_FISH_SMALL.getAnimationLength() - 470
                && attack.judgementTarget == null
                && !attack.triggered) {
            if (mount.level() instanceof ServerLevel serverLevel) {
                List<EntityType<? extends LivingEntity>> ENTITY_TYPES =
                        List.of(ErsEntities.MANDGEMARE_LABIUM.get(), ErsEntities.MAGNIDISCUMYZON_SARCOPTERUS.get());
                attack.judgementTarget = Objects.requireNonNull(ENTITY_TYPES
                        .get(mount.getRandom().nextInt(ENTITY_TYPES.size()))
                        .create(serverLevel));
        ...
        if (attack.animatorTick == 0) {
            mount.getAnimatableInstanceCache()
                    .getManagerForId(mount.getId())
                    .stopTriggeredAnimation("catch_fish-small");
        }
    }

    public void executeCatchFishMiddle(MobAttack attack, DentisaurusLongirostrisEntity mount) {
        if (attack.getType() != AttackType.SWAMP_DRAGON_CATCH_FISH_MIDDLE) return;

        if (attack.animatorTick == AttackType.SWAMP_DRAGON_CATCH_FISH_MIDDLE.getAnimationLength() - 470
                && attack.judgementTarget == null
                && !attack.triggered) {
            if (mount.level() instanceof ServerLevel serverLevel) {
                attack.judgementTarget = Objects.requireNonNull(
                        List.of(ErsEntities.ARGENTUMNISCUS_ACICULAULAR.get(), ErsEntities.LATIMERIA_PERCOIDES.get())
                                .get(mount.level().random.nextInt(2))
                                .create(serverLevel));
                serverLevel.addFreshEntity(attack.judgementTarget);
                attack.triggered = true;
            }
    ...
    public void executeJumpAttack(MobAttack attack, DentisaurusLongirostrisEntity mount) {
            ...
            for (Entity target : entities) {
                ...
                if (target instanceof LivingEntity livingEntity) {
                    ...
                    livingEntity.addDeltaMovement(
                            livingEntity.getDeltaMovement().add(lookVec.x * 1.5, 0.5, lookVec.z * 1.5));
                    ...
                    if (livingEntity instanceof ErsTamableVehicle<?> tamable) {
                        ...
                        if (tamable.isBaby() || (tamable.doAgeTick() && tamable.getAgeInDays() < 20)) {
                            livingEntity.addDeltaMovement(
                                    livingEntity.getDeltaMovement().add(lookVec.x * 1.5, 1, lookVec.z * 1.5));
                            return;
                        }
    ...
    public void executeAfterJumpLand(MobAttack attack, DentisaurusLongirostrisEntity mount) {
        if (attack.animatorTick < AttackType.SWAMP_DRAGON_AFTER_JUMP_LAND.getAnimationLength() - 22
                && mount.isInWater()) {
