...
public class AquicornisDinosauriformisBodyControl extends GeneralVehicleBodyControl {
    private final AquicornisDinosauriformisEntity entity;

    public AquicornisDinosauriformisBodyControl(AquicornisDinosauriformisEntity entity) {
        super(entity);
        this.entity = entity;
    }
    ...
    public void clientTick() {
        boolean isVehicle = this.vehicleTick(4.5f, 5f, 13, 11f, 9, 25, 75, 80);

        if (isVehicle) return;

        if (entity.updateSkyBrightness() < 4
                && !entity.isInWater()
            ...
            aiTick((float) this.entity.getMaxHeadYRot(), 2.5f, entity.isInWater() ? 2.7f : 10f);
    }
}
