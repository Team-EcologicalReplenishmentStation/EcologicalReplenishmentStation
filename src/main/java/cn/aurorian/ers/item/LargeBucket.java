   ...
   public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level pLevel, Player pPlayer, @NotNull InteractionHand pHand) {
      ...
      if (blockhitresult.getType() == HitResult.Type.MISS) {
         ...
         if (pLevel.mayInteract(pPlayer, blockpos) && pPlayer.mayUseItemAt(blockpos1, direction, itemstack)) {
               ...
               // 检查方块是否为水并且可以被捡起
               if (blockstate.getBlock() instanceof BucketPickup bucketpickup && blockstate.getFluidState().is(Fluids.WATER)) {
                   // 尝试捡起方块
                  ...
                  if (!pickedUpFluid.isEmpty()) {
                     ...
                     bucketpickup.getPickupSound(blockstate).ifPresent((sound) -> pPlayer.playSound(sound, 1.0F, 1.0F));
