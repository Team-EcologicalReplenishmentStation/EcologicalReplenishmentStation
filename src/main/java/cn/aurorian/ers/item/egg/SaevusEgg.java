...
public class SaevusEgg extends ErsEgg{
    ...
    public ErsTamable<?> born(Level level) {
        TerridensaurusSaevusEntity entity1 = ErsEntities.TERRIDENSAURUS_SAEVUS.get().create(level);
        if(entity1 != null) {
            if (level.random.nextFloat() < 0.3f) {
                entity1.setCanBeElite(true);
            }
            if (level.random.nextFloat() < 0.1f) {
                entity1.setVariant(TerridensaurusSaevusEntity.Variant.getRareSpawnVariant(level.random));
            } else {
                entity1.setVariant(TerridensaurusSaevusEntity.Variant.getCommonSpawnVariant(level.random));
            }
            return entity1;
        }
        return null;
