...
public class ErsCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("ers")
                ...
                .then(Commands.literal("elite")
                        .then(Commands.argument("target", EntityArgument.entity())
                                .executes(context -> setElite(context, EntityArgument.getEntity(context, "target")))))
                .then(Commands.literal("tame")
                        .then(Commands.argument("target", EntityArgument.entity())
                                .executes(context -> setTame(context, EntityArgument.getEntity(context, "target")))))
                .then(Commands.literal("variant")
                        .then(Commands.literal("set")
                                .then(Commands.argument("target", EntityArgument.entity())
                                        .then(Commands.argument("id", IntegerArgumentType.integer(0))
                                                .executes(context -> setVariant(
                                                        context,
                                                        EntityArgument.getEntity(context, "target"),
                        ...
                        .then(Commands.literal("get")
                                .then(Commands.argument("target", EntityArgument.entity())
                                        .executes(context ->
                                                getVariant(context, EntityArgument.getEntity(context, "target"))))))
                .then(Commands.literal("debug")
                        .then(Commands.argument("target", EntityArgument.entity())
                                .then(Commands.argument("state", StringArgumentType.string())
                                        .executes(context -> executeAttack(
                                                context, EntityArgument.getEntity(context, "target")))))));
    }

    private static int executeAttack(CommandContext<CommandSourceStack> context, Entity target) {
        if (target instanceof ErsTamable<?> vehicle) {
            String state = StringArgumentType.getString(context, "state");
            AttackType type = AttackType.fromAnimName(state);
            vehicle.startAttack(type);
            return 1;
        }
        return 0;
    }
    ...
    private static int setElite(CommandContext<CommandSourceStack> context, Entity target) {
        if (target instanceof ErsTamableVehicle<?> vehicle) {
            ...
            vehicle.updateAgeFromServer();
            context.getSource()
                    .sendSuccess(() -> Component.translatable("commands.ers.elite.success", target.getName()), true);
            return 1;
        } else {
            context.getSource().sendFailure(Component.translatable("commands.ers.elite.failed", target.getName()));
            return 0;
        }
    }

    private static int setTame(CommandContext<CommandSourceStack> context, Entity target)
    ...
    private static int setVariant(CommandContext<CommandSourceStack> context, Entity target, int variant) {
        if (context.getSource().isPlayer()
                && target instanceof ErsTamableVehicle<?> tamable
                && target instanceof VariantHolder<?>) {
    ...
    private static int getVariant(CommandContext<CommandSourceStack> context, Entity target) {
        if (context.getSource().isPlayer()
            ...
            context.getSource().sendSuccess(() -> Component.literal("Variant ID: " + variant), true);
            return 1;
        }
        return 0;
    }
    ...
    private static int setAge(CommandContext<CommandSourceStack> context, Entity target, int days) {
        if (target instanceof ErsTamable<?> ersTamable) {
            ...
            ersTamable.updateAgeFromServer();
            context.getSource()
                    .sendSuccess(
                            () -> Component.translatable("commands.ers.age.success", target.getName(), days), true);
            return 1;
        } else {
            context.getSource().sendFailure(Component.translatable("commands.ers.age.failed", target.getName()));
            return 0;
        }
    }
}
