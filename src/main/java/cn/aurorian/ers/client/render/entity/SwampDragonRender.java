...
import cn.aurorian.ers.packet.SwampDragonTrackFoodPacket;
    ...
    public void renderFinal(PoseStack poseStack, DentisaurusLongirostrisEntity animatable, BakedGeoModel model,
        ...
        super.renderFinal(poseStack, animatable, model, bufferSource, buffer, partialTick, packedLight, packedOverlay, red,
                ...
                model.getBone("Food").ifPresent(bone ->{
                    if (bone.getName().equals("Food") && !animatable.getAttackState().isEmpty()) {
                        //向服务器发包
                        ErsNetwork.INSTANCE.sendToServer(new SwampDragonTrackFoodPacket(animatable.getId(), new Vector3f((float)bone.getLocalPosition().x, (float)bone.getLocalPosition().y, (float)bone.getLocalPosition().z)));
                    }
                });
    }
}
