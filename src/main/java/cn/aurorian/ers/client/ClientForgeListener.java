...
@Mod.EventBusSubscriber(modid = EcologicalReplenishmentStation.MODID, bus = Mod.EventBusSubscriber.Bus.FORGE, value = Dist.CLIENT)
public class ClientForgeListener {
    ...
    public static void onKeyInput(InputEvent.Key event) {
        ...
        if (player != null && player.getVehicle() instanceof ErsTamableVehicle<?> tamable && tamable.getAttackState().isFinished()) {
            ...
            if (key == ErsKeyBindings.ATTACK_KEY.getKey().getValue()) {
                if(!ErsUtils.isMoving(tamable) && ErsKeyBindings.ATTACK4_KEY.isDown()){
                    ...
                    packet1 = new MobAttackPacket(player.getVehicle().getId(), 1);
    ...
    public static void onSprint(TickEvent.ClientTickEvent event) {
        ...
        if (player != null && player.getVehicle() instanceof ErsTamableVehicle<?> tamable) {
            ...
            if(triggerTime > 0 && !Minecraft.getInstance().options.keySprint.isDown() && !Minecraft.getInstance().options.keyUp.isDown()) {
                triggerTime --;
            }else if(triggerTime <= 0 && (Minecraft.getInstance().options.keySprint.isDown() || Minecraft.getInstance().options.keyUp.isDown())) {
                triggerTime = 6;
            }
            boolean currentState = Minecraft.getInstance().options.keySprint.isDown() || (triggerTime > 0 && triggerTime != 6) && Minecraft.getInstance().options.keyUp.isDown();
